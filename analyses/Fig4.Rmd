...
prev_stage <- select(hosts_per_stage, Parasite.species, Host.no, 
  ...
  mutate(log_prev_hosts = log(prev_hosts))
...
```
```{r}
reg9f <- glmer(num_hosts_suspicious_removed ~ zstudy_effort + 
                 # Def.int*Host_no_fac + 
                 log_prev_hosts*Host_no_fac*Def.int +
                 (1|Parasite.species) + (1|parasite_genus) + (1|parasite_family) +
                 (1|parasite_order) + (1|parasite_class) + (1|parasite_phylum) + (1|obs),
            data = filter(hosts_per_stage, Facultative != 'postcyclic', !is.na(num_hosts_suspicious_removed), !is.na(prev_hosts) ),
            family = 'poisson'
            )
```

`glmer` is slow. This is ok for fitting model, but not for bootstrapping. Get CIs from MCMC model
...
datx <- filter(hosts_per_stage, Facultative != 'postcyclic', !is.na(num_hosts_suspicious_removed), !is.na(prev_hosts) )
datx$pred <- "no"

nd <- expand.grid(log_prev_hosts = log(
  seq(min(hosts_per_stage$num_hosts_suspicious_removed, na.rm = T),
      max(hosts_per_stage$num_hosts_suspicious_removed, na.rm = T), 
      by = 1)),
...
nd <- filter(nd,
             !(Host_no_fac == "2" & Def.int == 'def' & exp(log_prev_hosts) > 36), 
             !(Host_no_fac == "2" & Def.int == 'int' & exp(log_prev_hosts) > 49), 
             !(Host_no_fac == "3" & Def.int == 'def' & exp(log_prev_hosts) > 30), 
             !(Host_no_fac == "3" & Def.int == 'int' & exp(log_prev_hosts) > 104), 
             !(Host_no_fac == "4" & Def.int == 'def' & exp(log_prev_hosts) > 31), 
             !(Host_no_fac == "4" & Def.int == 'int' & exp(log_prev_hosts) < 17),
             !(Host_no_fac == "4" & Def.int == 'int' & exp(log_prev_hosts) > 23)
             )
nd$pred <- "yes"
...
```{r}
reg9f_mc <- MCMCglmm(num_hosts_suspicious_removed ~ zstudy_effort +
                       # Def.int*Host_no_fac + 
                       log_prev_hosts*Host_no_fac*Def.int,
...
```{r}
cbind(fixef(reg9f), summary(reg9f_mc$Sol)$statistics[,1])
```
```{r}
summary(reg9f)
```
...
Here are the random effects `lmer` vs `MCMCglmm`. Again, the model is parameterized differently, but the CI is plausible.

```{r}
vc <- as.data.frame(VarCorr(reg9f))$vcov
cbind(round(vc,3), round(summary(reg9f_mc$VCV)$statistics[,1],3))
rm(vc)
```
...
f3a <- ggplot(filter(hosts_per_stage, Facultative != 'postcyclic', !is.na(hsi_lcdb_suspcious_rem), !is.na(prev_hosts) ),
  ...
  # PREDS MCMC
  geom_ribbon(data = filter(p, !(Host_no_fac == 4 & Def.int == "int")),
              aes(x = exp(log_prev_hosts), y = fit, fill = di2,
                  ymin = lwr, ymax = upr), 
              alpha = 0.2, color = NA) +
  geom_line(data = filter(p, !(Host_no_fac == 4 & Def.int == "int")),
            aes(x = exp(log_prev_hosts), y = fit, color = di2), 
...
```{r}
reg9fx <- lmer(hsi_lcdb_suspcious_rem ~ zstudy_effort + prev_hsi*Host_no_fac*Def.int +
                 (1|Parasite.species) + (1|parasite_genus) + (1|parasite_family) +
...
```{r}
# make data where we want predicted values
datx2 <- filter(hosts_per_stage, Facultative != 'postcyclic', !is.na(hsi_lcdb_suspcious_rem), !is.na(prev_hosts) )
datx2$pred <- "no"
...
```{r}
reg9fx_mc <- MCMCglmm(hsi_lcdb_suspcious_rem ~ zstudy_effort +
                       prev_hsi*Host_no_fac*Def.int,
                   random = ~ Parasite.species + parasite_genus + parasite_family + 
                     parasite_order + parasite_class + parasite_phylum,
            data = datx2,
            family = 'gaussian',
...
```{r}
cbind(fixef(reg9fx), summary(reg9fx_mc$Sol)$statistics[,1])
```
...
```{r}
p <- predict.MCMCglmm(reg9fx_mc,
                      type = "terms",
                      interval = "confidence",
                      marginal = ~ Parasite.species + parasite_genus + parasite_family + parasite_order +
                        parasite_class + parasite_phylum)
p <- bind_cols(datx2, data.frame(p))
p <- filter(p, pred == "yes")
p <- mutate(p, lwr = if_else(lwr < 1, 1, lwr))
```
...
f3b <- ggplot(filter(hosts_per_stage, Facultative != 'postcyclic', !is.na(hsi_lcdb_suspcious_rem), !is.na(prev_hsi) ),
  ...
  # PRED MCMC GLMM
  geom_ribbon(data = filter(p, !(Host_no_fac == 4 & Def.int == "int")),
              aes(x = prev_hsi, y = fit, fill = di2,
                  ymin = lwr, ymax = upr), 
              alpha = 0.3, color = NA) +
  geom_line(data = filter(p, !(Host_no_fac == 4 & Def.int == "int")),
            aes(x = prev_hsi, y = fit, color = di2), 
            size = 1, linetype = 'solid') +
...
```{r}
f3c <- plot_grid(f3a2, f3b2, ncol = 1, align="hv")
```
...
ggsave(filename = "../figs/Fig4_rev.svg", plot = f3c, device = 'svg', units = 'in', width = fig_width, height = fig_height*2)
ggsave(filename = "../figs/Fig4_rev.png", plot = f3c, device = 'png', units = 'in', width = fig_width, height = fig_height*2)
```
