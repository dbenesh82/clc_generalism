...
figsx
ggsave(figsx, filename = "../../figs/FigS7.svg", device = 'svg', width = 6, height = 4)
ggsave(figsx, filename = "../../figs/FigS7.png", device = 'png', width = 6, height = 4)
rm(figsx)
```
...
ggplot(filter(chains3_l, hosts != 0, web_study %in% most_relevant_studies2),
  ...
  scale_color_distiller(type = 'div', palette = 'RdBu') +
  theme(panel.grid.minor = element_blank()) +
  labs(x = "Sequential predation events (host in cycle)", y = "Proportion of encountered hosts", color = "Proportion primary\nconsumers in food web\ninfected as first hosts")
```
...
f2u <- ggplot(filter(hosts_per_stage, Facultative != 'postcyclic', Host.no != 5), # post-cyclic hosts removed
  ...
  labs(x = "Stage (host) in life cycle", y = "Host records", color = "Proportion\nfirst hosts") +
  theme(
  ...
  guides(fill = FALSE)
f2u
ggsave(f2u, filename = "../../figs/FigS9a.png", device = 'png', units = 'in', width = 6, height = 5)
ggsave(f2u, filename = "../../figs/FigS9a.svg", device = 'svg', units = 'in', width = 6, height = 5)
```
...
figsx <- ggplot(filter(chains3_l, hosts !=0, web_study %in% most_relevant_studies2),
  ...
  theme(panel.grid.minor = element_blank(), 
        ...
        strip.text = element_text(size = 6))
figsx
ggsave(figsx, filename = "../../figs/FigS8.svg", device = 'svg', width = 6, height = 4)
ggsave(figsx, filename = "../../figs/FigS8.png", device = 'png', width = 6, height = 4)
rm(figsx)
```
...
f2l <- ggplot(filter(hosts_per_stage, Facultative != 'postcyclic', Host.no != 5), # post-cyclic hosts removed
  ...
  scale_color_distiller(type = 'div', palette = 'RdBu') +
  scale_fill_brewer(type = "qual", palette = "Set2") +
  labs(x = "Stage (host) in life cycle", y = "Taxonomic dissimilarity", color = "Proportion\nfirst hosts") +
  theme(
        # legend.position = c(1,1),
        # legend.justification = c(1,1),
        legend.background = element_rect(color = 'black'),
  ...
  guides(fill = FALSE)
f2l
ggsave(f2l, filename = "../../figs/FigS9b.svg", device = 'svg', units = 'in', width = 6, height = 5)
ggsave(f2l, filename = "../../figs/FigS9b.png", device = 'png', units = 'in', width = 6, height = 5)
```

Then to mirror figure 3, we put the two plots with expectations over observations together and export it.
...
ap <- align_plots(f2u, f2l, align="hv")
```
```{r}
laff3 <- gridExtra::grid.arrange(ap[[1]], ap[[2]])
```

```{r}
ggsave(filename = "../../figs/FigS9.svg", plot = laff3, units = "in", width = 8, height = 8) # bringing the upper plot down still
```
```{r}
rm(f2u, f2l, laff3, ap)
```

Additionally, in Figure 1, we want to use the Carpinteria Salt Marsh food web as an illustration for how generalism changes with successive transmission steps. To that end, we plot host range and taxonomic dissimilarity for simulated cycles for just this food web, stack them, and export the figure.
```{r}
...
laff1 <- ggplot(filter(chains3_l, hosts != 0, web_study == "Lafferty et al. (2006)"),
  ...
  theme(panel.grid.minor = element_blank()) +
  guides(color = FALSE) +
  annotate(geom = 'text', label = "(b)", x = 4, y = 60)
...
laff2 <- ggplot(filter(chains3_l, hosts !=0, web_study == "Lafferty et al. (2006)"),
  ...
  theme(panel.grid.minor = element_blank(),
        ...
        legend.background = element_rect(color = 'black')) +
  annotate(geom = 'text', label = "(c)", x = 4, y = max(chains3_l$hsi, na.rm = T))
...
laff1 <- laff1 + 
  theme(axis.title.x = element_blank(), 
        axis.text.x = element_blank())
ap <- align_plots(laff1, laff2, align="hv")
```
```{r}
laff3 <- gridExtra::grid.arrange(ap[[1]], ap[[2]])
```
...
```{r}
# This is Fig 1b and c!
ggsave(filename = "../../figs/Fig1bc.svg", plot = laff3, units = "in", width = 5, height = 8) # bringing the upper plot down still
```
```{r}
rm(laff1, laff2, laff3, ap)
```
...
figsx <- ggplot(filter(chains3_l, hosts!=0, web_study %in% most_relevant_studies2),
  ...
  labs(x = "Sequential predation events (host in cycle)", y = "Cumulative encountered hosts", 
       color = "Proportion primary\nconsumers in food web\ninfected as first hosts")
figsx
ggsave(figsx, filename = "../../figs/FigS4.png", device = 'png', width = 6, height = 4)
ggsave(figsx, filename = "../../figs/FigS4.svg", device = 'svg', width = 6, height = 4)
rm(figsx)
```
...
f1a <- ggplot(dat, aes(y = num_hosts_lcdb_nhm, x = lcl_max_fac)) +
  ...
  geom_line(data = d_out,
            aes(x = factor(Host.no), y = hosts, group = perc, color = perc, fill = NULL),
            linetype = 'solid', alpha = 0.5) +
  scale_y_log10(limits = c(0.95,max(dat$num_hosts_lcdb_nhm)), breaks = c(1, 5, 10, 25, 50, 100, 200, 400)) +
  scale_color_distiller(type = 'div', palette = 'RdBu') +
  labs(x = "Life cycle length (max)", y = "Host range", size = "Pubmed hits", color = "Proportion\nfirst hosts") +
  guides(size = FALSE, color = FALSE) +
  theme(panel.grid.minor = element_blank(),
        panel.grid.major.x = element_blank()) +
  annotate(geom = 'text', label = "(a)", x = 0.55, y = max(dat$num_hosts_lcdb_nhm))
f1a
ggsave(f1a, filename = "../../figs/FigS6a.svg", device = 'svg', units = 'in', width = 4.5, height = 4.5)
ggsave(f1a, filename = "../../figs/FigS6a.png", device = 'png', units = 'in', width = 4.5, height = 4.5)
```
...
figsx <- ggplot(filter(chains3_l, !is.na(hsi), web_study %in% most_relevant_studies2),
  ...
  scale_color_distiller(type = 'div', palette = 'RdBu') +
  scale_y_continuous(limits = c(1,6), breaks = c(1:6), labels = c("species", tax.ranks)) +
  labs(x = "Sequential predation events (host in cycle)", y = "Cumulative taxonomic dissim index", 
       color = "Proportion primary\nconsumers in food web\ninfected as first hosts")
figsx
...
f1b
ggsave(f1b, filename = "../../figs/FigS6b.svg", device = 'svg', units = 'in', width = 4.5, height = 4.5)
ggsave(f1b, filename = "../../figs/FigS6b.png", device = 'png', units = 'in', width = 4.5, height = 4.5)
```
...
s7a
ggsave(s7a, filename = "../../figs/FigS10a.svg", device = 'svg', height = 4, width = 4)
```
...
s7b <- ggplot(filter(chains3, web_study == "Lafferty et al. (2006)", third_hosts != 0),
  ...
  guides(color = F)
s7b
ggsave(s7b, filename = "../../figs/FigS10b.svg", device = 'svg', height = 4, width = 4)
```
...
s7c <- ggplot(filter(chains3, web_study == "Lafferty et al. (2006)", second_hosts != 0),
  ...
  guides(color = F)
s7c
ggsave(s7c, filename = "../../figs/FigS10c.svg", device = 'svg', height = 4, width = 4)
```
```{r}
s7d <- ggplot(filter(chains3, web_study == "Lafferty et al. (2006)", second_hosts != 0),
  ...
  guides(color = F)
s7d
ggsave(s7d, filename = "../../figs/FigS10d.svg", device = 'svg', height = 4, width = 4) # combine S7 a to d into one plot
```
