...
The reviewer was also interested in the "evenness" of host taxonomy, so we can plot that across different life cycle lengths

```{r}
f1c <- ggplot(filter(dat, num_hosts_lcdb_nhm > 2),
              aes(y = var.hsi_comb, x = lcl_max_fac)) +
  geom_boxplot(outlier.color = NA) +
  ...
  labs(x = "Life cycle length (max)", y = "Unevenness in host diversity", size = "Pubmed hits") +
  guides(size = FALSE) +
  theme(panel.grid.minor = element_blank(),
        panel.grid.major.x = element_blank()) 
f1c
...
# fig_height <- 3.5
# 
# ggsave(filename = "../figs/Fig2_tax_evenness.png", plot = f1c, device = 'png', units = 'in', 
#        width = fig_width, height = fig_height) 

```

Let's also make the first two figures, but with the predicted values from mixed model controlling for study effort and taxonomy. I'll fit the model both with `lmer` and `MCMCglmm`, because I want to add confidence intervals to the predicted means. This is easier with a Bayesian approach, but I also bootrstrapped the `lmer` model and it gave quite comparable CIs. The Bayesian approach is also faster, because bootstrapping the model takes a long time.
...
nd$zstudy_effort <- 0
nd$parasite_genus <- dat$parasite_genus[1]
nd$parasite_family <- dat$parasite_family[1]
nd$parasite_order <- dat$parasite_order[1]
nd$parasite_class <- dat$parasite_class[1]
nd$parasite_phylum <- dat$parasite_phylum[1]
nd$pred <- "yes"
datx <- bind_rows(datx, nd)
```

```{r}
regfmc <- MCMCglmm(num_hosts_lcdb_nhm ~ zstudy_effort + lcl_max_fac,
                   random = ~ parasite_genus + parasite_family + parasite_order +
...
Compare parameters from two models - basically the same.
...
```{r}
p <- predict.MCMCglmm(regfmc,
                      type = "terms",
                      interval = "confidence",
                      marginal = ~ parasite_genus + parasite_family + parasite_order +
                        parasite_class + parasite_phylum)
...
```{r}
# # bootstrap lmer model - takes a long time
# nd <- select(dat, lcl_max_fac)%>%arrange(lcl_max_fac)%>%distinct()
# nd$zstudy_effort <- 0
# nd$num_hosts_lcdb_nhm <- 0
# nd$lcl <- 1:4
# 
# pd <- model.matrix(terms(regf), nd)
# 
...
#     c(beta=fixef(.))
# }
# bo <- bootMer(regf, mySumm, nsim = 100) # bootstrap the model
# bx <- as.data.frame(bo)
...
# names(boots) <- as.character(c(1:100))
# boots <- bind_cols(nd, boots)
# boots <- pivot_longer(boots, `1`:`100`)
# 
...
f1a2 <- ggplot(dat, aes(y = num_hosts_lcdb_nhm, x = lcl_max_fac)) +
  ...
  theme(panel.grid.minor = element_blank(),
        panel.grid.major.x = element_blank()) +
  annotate(geom = 'text', label = "(a)", x = 0.55, y = max(dat$num_hosts_lcdb_nhm)) +
  geom_line(data = p, aes(x = lcl, y = fit), 
...
```{r}
regfxmc <- MCMCglmm(hsi_comb ~ zstudy_effort + lcl_max_fac,
                   random = ~ parasite_genus + parasite_family + parasite_order +
                     parasite_class + parasite_phylum,
            data = datx,
            family = 'gaussian',
...
```{r}
p <- predict.MCMCglmm(regfxmc,
                      type = "terms",
                      interval = "confidence",
                      marginal = ~ parasite_genus + parasite_family + parasite_order +
                        parasite_class + parasite_phylum)
p <- bind_cols(datx, data.frame(p))
p <- filter(p, pred == "yes")
p$lcl <- 1:4
```
...
# nd$hsi_comb <- 0
# nd$lcl <- 1:4
# pd <- model.matrix(terms(regfx), nd)
# 
...
#     c(beta=fixef(.))
# }
# bo <- bootMer(regfx, mySumm, nsim = 1000) # bootstrap the model
# bx <- as.data.frame(bo)
# bx <- select(bx, starts_with('beta'))
# bx <- as.matrix(bx)
# 
# boots <- bx %*% t(pd) # predicted values for every observation for every bootstrap
# boots <- as.data.frame(t(boots))
# names(boots) <- as.character(c(1:1000))
# boots <- bind_cols(nd, boots)
# boots <- pivot_longer(boots, `1`:`1000`)
# 
# # first get predicted size at each stage for every bootstrap
# boot_means2 <- boots%>%
#   group_by(lcl_max_fac)%>%
...
f1b2 <- ggplot(dat, aes(y = hsi_comb, x = lcl_max_fac)) +
  ...
  #           alpha = 1, size = 1.5, color = 'black', linetype = 'dashed') +
  # geom_pointrange(data = nd, 
  #                 aes(x = lcl, y = preds,
  #                     ymin = low_rg, ymax = high_rg), 
  #           size = 1, shape = 23, color = 'black', fill = 'black')
...
ggsave(filename = "../figs/Fig2_comb.png", plot = f1, device = 'png', units = 'in', width = fig_width*2, height = fig_height) 
ggsave(filename = "../figs/Fig2_comb.svg", plot = f1, device = 'svg', units = 'in', width = fig_width*2, height = fig_height) 
```
