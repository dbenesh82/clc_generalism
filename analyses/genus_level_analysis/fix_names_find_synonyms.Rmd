...
dat_nhm <- read.csv(file = "../../data/NHM_db_hosts_genus.csv", header = TRUE)
```
...
names_df_col <- mutate(names_df_col, query_genus = if_else(!grepl(pattern = " ", query), query,
  ...
  filter( !(grepl('\\(', query) | grepl('\\[', query)) )
names_df_col$db <- "col"

```

```{r}
already_done <- read.csv("../../data/host_names_corrected_valid.csv")
```


```{r}
names_df_col <- mutate(names_df_col, already_queried = if_else(query %in% already_done$query_name, 1, 0)) # which names already queried?
names_df_col <- left_join(names_df_col, select(already_done, query_name, col_id)%>%distinct(), by = c("query" = "query_name")) # add col ids to existing

mv <- which(names_df_col$already_queried==0)
corrected_names <- names_df_col$query[mv]
```
...
names_df_col$col_id[mv] <- col_ids
```
...
names_df_col <- left_join(names_df_col, sp_tax_wide, by = c("col_id" = "query_id"))
...
```{r}
mv <- which(names_df_col$already_queried==1 & !is.na(names_df_col$col_id))

names_df_col$species[mv] <- already_done[match(names_df_col$col_id[mv], already_done$col_id, ), 'col_species']
names_df_col$genus[mv] <- already_done[match(names_df_col$col_id[mv], already_done$col_id, ), 'col_genus']
names_df_col$family[mv] <- already_done[match(names_df_col$col_id[mv], already_done$col_id, ), 'col_family']
names_df_col$order[mv] <- already_done[match(names_df_col$col_id[mv], already_done$col_id, ), 'col_order']
names_df_col$class[mv] <- already_done[match(names_df_col$col_id[mv], already_done$col_id, ), 'col_class']
names_df_col$phylum[mv] <- already_done[match(names_df_col$col_id[mv], already_done$col_id, ), 'col_phylum']
names_df_col$kingdom[mv] <- already_done[match(names_df_col$col_id[mv], already_done$col_id, ), 'col_kingdom']
names_df_col$changed_name[mv] <- already_done[match(names_df_col$col_id[mv], already_done$col_id, ), 'changed_name']
names_df_col$valid_name[mv] <- already_done[match(names_df_col$col_id[mv], already_done$col_id, ), 'col_valid_name']
```
...
dataH <- left_join(arrange(dataH, Host.species),
                   select(names_df_col, query, db, col_id,
...
```{r}
write.csv(dataH, file = "../../data/host_names_corrected_valid_genus.csv", row.names = F)
```
...
```{r}
write.csv(dat_lcdb_nhm, file = "../../data/host_parasite_records_cleaned_names2.csv", row.names = F)
```
```{r}
save.image("workspace_after_tax_corrections.RData")
```
