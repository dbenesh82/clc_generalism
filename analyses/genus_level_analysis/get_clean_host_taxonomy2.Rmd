...
```{r}
dat_host <- read.csv(file="../../data/host_names_corrected_valid_genus.csv", header = TRUE, sep=",") # corrected host names
dat_host <- mutate(dat_host, best_host_name_genus = unlist(strsplit(as.character(best_host_name), split=" .*$")))
```

```{r}
already_done <- read.csv(file="../../data/ncbi_host_taxonomy.csv", header = TRUE, sep=",") # corrected host names
```

```{r}
dat_host_done <- filter(dat_host, best_host_name %in% already_done$sp.query)
dat_host_todo <- filter(dat_host, !best_host_name %in% already_done$sp.query)
```


```{r}
# get the set of host species and host genera
h.sp.vec <- unique(dat_host_todo$best_host_name)
h.sp.vec <- sort(na.omit(h.sp.vec))

g.sp.vec <- unique(dat_host_todo$best_host_name_genus)
...
# s4 <- system.time(ids.for.classif<-get_uid_(h.sp.vec[1:200]))
# 
# tst <- data.frame(sp = c(10,50,100,200), time = c(s1['elapsed'], s2['elapsed'], s3['elapsed'], s4['elapsed']))
...
We query the NCBI database to get the uniques ids for each. It takes a while. The last time this script was run and the database was queried was `r Sys.time()`.
...
for(i in 4555:length(ids.for.classif)) {
...
What proportion of names had a hit?

```{r}
length(unique(m.df$sp.query))/length(h.sp.vec)
```
...
The proportion of hosts in the database for which there was a hit in the NCBI database was `r round( dim(ncbi.tax.out)[1] / length(h.sp.vec), 2)`. However, the downloaded taxonomies are not consistent across all hosts species. For example, when we just look at the classical taxonomic hierarchy (genus, family, order, class, phylum), we see quite a few host species are missing order or class.
...
```{r, message=FALSE, warning=FALSE}
source("../get_host_taxonomy/impute_tax_levels.R") # load functions for imputing

ncbi.tax.out2 <- ncbi.tax.out #make a copy to check how much was filled in
ncbi.tax.out$subkingdom <- NA
...
for(i in seq_along(tax.ranks)){ #the first loop goes across main taxonomic columns in ncbi.tax.out
  ...
  for(j in seq_along(ncbi.tax.out$i.vec)){ #the second loop goes through the rows of main columns
    ...
    if( is.na(ncbi.tax.out[j, rank])) {
      ...
      # this may throw an error if tax levels are missing in main dataset (i.e. no parvorder column)
...
```{r}
# add in host names from records in DBs
ncbi.tax.out <- left_join(ncbi.tax.out, select(dat_host, host_record_lcdb_nhm, best_host_name), by = c("sp.query" = "best_host_name"))
```
```{r}
# combine previously run and newly downloaded taxonomies
ncbi.tax.out <- rename(ncbi.tax.out, species.group = `species group`)%>%
  select(names(already_done))

already_done <- mutate(already_done, ncbi.ids = as.character(ncbi.ids))


ncbi.tax.out <- bind_rows(already_done, ncbi.tax.out)%>%distinct()
```

```{r, message=FALSE, warning=FALSE}
rm(common.tax.groups, i, j, ncbi.tax.out2, rank, tax.ranks)

write.table(ncbi.tax.out, file = "../../data/ncbi_host_taxonomy2.csv", sep = ",", row.names = F) 
```
