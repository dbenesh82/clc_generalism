...
```
```{r}
anova(reg1, reg2, reg3, reg3.1, reg4)
```
...
```
```{r}
mod_list <- list(reg1, reg2, reg3, reg3.1, reg4)
...
reg4t <- update(reg3.1t, . ~ . + lcl_min) # add min life cycle length
```
```{r}
...
syb
# ggsave(syb, filename = "../../figs/FigSYb.png", device = 'png', width = 4.5, height = 4.5)
ggsave(syb, filename = "../../figs/FigSYb.svg", device = 'svg', width = 4.5, height = 4.5)
```
