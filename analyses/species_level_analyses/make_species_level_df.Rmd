...
dat_host_red <- filter(dat_host, !is.na(Host.species), !is.na(Host.common.name))%>%
  select(Parasite.species, Host.species, best_host_name)
dat_nhm_red <- select(dat_nhm, Parasite.species, Host.species, best_host_name)
```
```{r}
# this writes a combined set of host records for dryad
dat_host_red2 <- dat_host_red
dat_nhm_red2 <- dat_nhm_red
dat_host_red2$source <- 'lcdb'
dat_nhm_red2$source <- 'nhm'
dat_lcdb_nhm2 <- bind_rows(dat_host_red2, dat_nhm_red2) # combine 
dat_lcdb_nhm2 <- arrange(dat_lcdb_nhm2, Parasite.species, best_host_name)
# write.csv(dat_lcdb_nhm2, file = "../../data/host_records_lcdb_nhm_combined.csv", row.names = F)
