...
syc <- ggplot(r2_tax, aes(x = levels, y = tax_var_explained, color = approach)) +
  ...
  guides(color = FALSE)
syc
# ggsave(syc, filename = "../../figs/FigS2c.png", device = 'png', width = 4.5, height = 4.5)
ggsave(syc, filename = "../../figs/FigS2c.svg", device = 'svg', width = 4.5, height = 4.5)
```
...
sx <- ggplot(filter(hosts_per_stage, parasite_family %in% rx$family[1:12], Facultative != 'postcyclic')%>%
  ...
  theme(panel.grid.minor = element_blank(),
        panel.grid.major.x = element_blank())
ggsave(sx, filename = "../../figs/FigS3.png", device = 'png', width = 7, height = 7)
```
