...
```{r}
# # write simple data table for dryad
# hx <- hosts_per_stage%>%
#   filter(Facultative != 'postcyclic')%>%
#   rename(stage_length_end = stage_body_length, stage_biov_end = stage_biov)%>%
#   mutate(stage_length_start = stage_length_end - abs_growth_len,
#          stage_biov_start = stage_biov_end - abs_growth_biov)%>%
#   mutate(rg = log(stage_biov_end) - log(stage_biov_start))%>%
#   select(Parasite.species, parasite_genus, parasite_family, parasite_order, parasite_class, parasite_phylum,
#          Host_number = Host.no, Host_num_fac = Host_no_fac, Def.int, Facultative, 
#          lcl_max, lcl_min, lcl_max_fac, partial_cycle,
#          study_effort,
#          num_hosts_lcdb = num_hosts_suspicious_removed, 
#          hsi_lcdb = hsi_lcdb_suspcious_rem, var_hsi_lcdb = var.hsi_lcdb_suspcious_rem,
#          biov = stage_biov_end, dt = avg_dt, dd = avg_dd, rel_growth = rg )
# write.csv(hx, file = "../../ms_files/AmNat/first_revision/dryad_repo/stage_level_generalism.csv", row.names = F)
```
